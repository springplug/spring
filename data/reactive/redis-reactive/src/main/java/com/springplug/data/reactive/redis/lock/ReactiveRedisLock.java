...
package com.springplug.data.reactive.redis.lock;
 ...
 */
@Slf4j
public final class ReactiveRedisLock implements InitializingBean, DisposableBean {
	 ...
	 */
	public ReactiveRedisLock(ReactiveRedisConnectionFactory reactiveRedisConnectionFactory, String registryKey) {
	 ...
	 */
	public ReactiveRedisLock(ReactiveRedisConnectionFactory reactiveRedisConnectionFactory, String registryKey, Duration expireEvictIdle) {
	 ...
	 */
	public ReactiveRedisLock(ReactiveRedisConnectionFactory reactiveRedisConnectionFactory, String registryKey, Duration expireAfter, Duration expireEvictIdle) {
	...
	public ReactiveRedisDistributedLock obtain(String lockKey) {
	...
	public <T> Mono<T> doInTransaction(String transactionKey, Supplier<Mono<T>> supplier) {
		return obtain(transactionKey)
				.acquireAndExecute(() -> supplier.get())
	 ...
	 */
	private final class ReactiveRedisDistributedLock implements IReactiveRedisLock {
		...
		private String constructLockKey(String path) {
			return ReactiveRedisLock.this.registryKey + ':' + path;
		}
		...
		public String toString() {
			...
			return "ReactiveRedisLock [lockKey=" + this.lockKey
		...
		private ReactiveRedisLock getOuterType() {
			return ReactiveRedisLock.this;
		}
		...
		private Mono<Boolean> obtainLock() {
			return Mono.from(ReactiveRedisLock.this.reactiveStringRedisTemplate
							.execute(ReactiveRedisLock.this.obtainLockScript,
									Collections.singletonList(this.lockKey),
									List.of(this.lockId, String.valueOf(ReactiveRedisLock.this.expireAfter)))
					)
					.map(success -> {
		...
		private Mono<Boolean> removeLockKey() {
			return Mono.just(ReactiveRedisLock.this.unlinkAvailable)
					.filter(unlink -> unlink)
					.flatMap(unlink -> ReactiveRedisLock.this.reactiveStringRedisTemplate
							.unlink(this.lockKey)
							.doOnError(throwable -> {
								ReactiveRedisLock.this.unlinkAvailable = false;
							...
							.onErrorResume(throwable -> ReactiveRedisLock.this.reactiveStringRedisTemplate.delete(this.lockKey))
					)
					.switchIfEmpty(ReactiveRedisLock.this.reactiveStringRedisTemplate.delete(this.lockKey))
					.then(Mono.just(true));
		}
		...
		Mono<Boolean> isAcquiredInThisProcess() {
			return ReactiveRedisLock.this.reactiveStringRedisTemplate.opsForValue()
					.get(this.lockKey)
		...
		public Mono<Boolean> acquire() {
			...
			return this.acquire(ReactiveRedisLock.this.expireDuration);
		}
